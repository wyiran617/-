            ...
            <li v-for="item in list" :key="item.product_id">
                <el-popover placement="top">
                    ...
                    <div style="text-align: right; margin: 10px 0 0">
                        <el-button type="primary" size="mini" @click="deleteCollect(item.cid)">确定</el-button>
                    </div>
                    <i class="el-icon-close delete" slot="reference" v-show="isDelete"></i>
                </el-popover>
                <router-link :to="{ path: '/goods/details', query: {productID:item.id} }">
                    <img :src="'http://localhost' +item.image+'collect.png'" alt />
                    <h2>{{item.title}}</h2>
                    <h3>{{item.sellPoint}}</h3>
                    <p>
                        <span>{{item.price}}元</span>
<!--                        <span v-show="item.price != item.product_selling_price" class="del">-->
...
<script>
    export default {
        ...
        methods: {
            deleteCollect(cid) {
                axios.delete('http://localhost/tCollect/delete/'+cid)
                    .then(res => {
