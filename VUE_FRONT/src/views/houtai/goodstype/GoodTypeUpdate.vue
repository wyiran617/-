...
<script>
    export default {
        ...
        created() {
            let _this = this;
            axios.get('http://localhost/tGoodsCategory/'+this.$route.query.id).then(function (resp) {
                _this.form=resp.data;
                axios.get('http://localhost/tGoodsCategory/'+_this.form.parentId).then(function (resp) {
                    _this.showSelect=resp.data.name;
                    console.log(_this.showSelect)
                });
                console.log(_this.form);
                _this.radioIsParent =  _this.form.isParent;
        ...
        methods:{
            ...
            onSubmit(formName){
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        console.log(_this.form);
                        axios.put('http://localhost/tGoodsCategory',this.form).then(function (resp) {
                            console.log(resp);
                            if (resp.status === 200){
