        ...
        <div class="order-content" v-if="orders.length>0">
            <div class="content" v-for="(item,index) in orders" :key="index">
                <ul>
                    <!-- 我的订单表头 -->
                    <li class="order-info">
                        <div class="order-id">订单编号: {{oids[index]}}</div>
                        <div class="order-time">订单时间: {{createTime[index] | dateFormat}}</div>
                    </li>
                    <li class="header">
                    ...
                    <!-- 订单列表 -->
                    <li class="product-list" v-for="(item,i) in orders[index]" :key="i">
                            <div class="pro-img">
            ...
            <span class="order-total">
              共
              <span class="order-total-num">{{totalNum[index]}}</span> 件商品
            </span>
                    </div>
                    <div class="order-bar-right">
            <span>
              <span class="total-price-title">合计：</span>
              <span class="total-price">{{totalPrice[index]}}元</span>
            </span>
                    </div>
                    <!-- 订单列表END -->
                </div>
...
<script>
    export default {
        ...
        activated() {
            ...
            axios.get('http://localhost/tOrderItem/allByUid/'+uid)
                .then(res => {
                    if (res.data.code === "001") {
                        this.oids = res.data.data.oids;
                        ...
                        console.log("this.oids:"+this.oids);
