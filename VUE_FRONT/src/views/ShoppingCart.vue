    ...
    <div class="shoppingCart">
        ...
        <div class="content" v-if="getShoppingCart.length>0">
            <ul>
                ...
                <!-- 购物车列表 -->
                <li class="product-list" v-for="(item,index) in getShoppingCart" :key="item.pid">
                    <div class="pro-check">
                        <el-checkbox :value="item.isCheck" @change="checkChange($event,index)"></el-checkbox>
                    </div>
                    <div class="pro-img">
                    ...
                    <div class="pro-name">
                        <router-link
                                :to="{ path: '/goods/details', query: {productID:item.pid} }"
                    ...
                    <div class="pro-num">
                        <el-input-number
                                size="small"
                                :value="item.num"
                                @change="handleChange($event,index,item.pid)"
                                :min="1"
                                :max="item.maxNum"
                        ></el-input-number>
                    </div>
                        ...
                        <el-popover placement="right">
                            ...
                            <div style="text-align: right; margin: 10px 0 0">
                                <el-button
                                        type="primary"
                                        size="mini"
                                        @click="deleteItem($event,item.cid,item.pid)"
                                >确定</el-button>
                            </div>
    ...
    export default {
        ...
        methods: {
            ...
            handleChange(currentValue, key, productID) {
                ...
                this.toEditShopCart.uid = this.$store.getters.getUser.uid;
                this.toEditShopCart.pid = productID;
                this.toEditShopCart.num = currentValue;
                this.toEditShopCart.isCheck = key;
                console.log("key:"+key);
                axios.put("http://localhost/tCart/ByPidAndUid",this.toEditShopCart)
                    .then(res => {
            ...
            // 向后端发起删除购物车的数据库信息请求
            deleteItem(e, cid, productID) {
                axios.delete('http://localhost/tCart/delete/'+cid)
                    .then(res => {
                        switch (res.data.code) {
                            case "001":
                                // “001” 删除成功
                                // 更新vuex状态
                                this.deleteShoppingCart(cid);
                                // 提示删除成功信息
