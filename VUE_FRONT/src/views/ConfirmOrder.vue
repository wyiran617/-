            ...
            <div class="section-address">
                ...
                <div class="address-body">
                    <ul>
                        ...
                        <li class="add-address" @click="toInsertAddress">
                ...
                <div class="goods-list">
                    <ul>
                        <li v-for="item in getCheckGoods" :key="item.id">
                            <img :src="'http://localhost' + item.image+'collect.png'" />
...
<script>
    ...
    import RecvAddress from "../components/RecvAddress";
    export default {
        name: "",
        ...
        created() {
            ...
            axios.get('http://localhost/tAddress/allByUid/'+this.user.uid).then(function (resp) {
                ...
                let temp = _this.address[0];
                _this.OrderInfo.uid = _this.user.uid;
                _this.OrderInfo.recvName = temp.name;
                _this.OrderInfo.recvPhone = temp.phone;
                _this.OrderInfo.totalPrice = _this.getTotalPrice;
                _this.OrderInfo.recvProvince = temp.provinceName;
                _this.OrderInfo.recvCity = temp.cityName;
                _this.OrderInfo.recvArea = temp.areaName;
                _this.OrderInfo.recvAddress = temp.address;
                })
            this.createOrderNum();
            console.log(this.OrderInfo.oid)
        },
        computed: {
        ...
        methods: {
            ...
            createOrderNum(){
                ...
                prefixStr = year+month+day;
                this.OrderInfo.oid = prefixStr+numlist.join('');
            },
            addOrder() {
                let products = this.getCheckGoods;
                axios.post("http://localhost/tOrderItem/addBatch/"+this.OrderInfo.oid,products)
                    .then(res => {
                        switch (res.data.code) {
                            ...
                            case "001":
                                for (let i = 0; i < products.length; i++) {
                                    ...
                                    // 删除已经结算的购物车商品
                                    this.deleteShoppingCart(temp.cid);
                                    axios.delete('http://localhost/tCart/delete/'+temp.cid).then(res=>{
                ...
                axios.post("http://localhost/tOrder/", this.OrderInfo).then(res => {
